...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['number']) && isset($data['name']) && isset($data['money'])) {
        $number = $data['number'];
        $name = $data['name'];
        $money = $data['money'];

        // Prepare SQL statement
        $stmt = $conn->prepare("INSERT INTO simcards (number, name, money) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $number, $name, $money);
